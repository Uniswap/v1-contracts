...
@pytest.fixture
def exchange_template(get_contract):
    return get_contract(EXCHANGE_CODE)


@pytest.fixture
def omg_token(get_contract):
    return get_contract(ERC20_CODE, *[b"OMG Token", b"OMG", 18, 100000*10**18])


@pytest.fixture
def dai_token(get_contract):
    return get_contract(ERC20_CODE, *[b"DAI Token", b"DAI", 18, 100000*10**18])


@pytest.fixture
def exchange_factory(get_contract, exchange_template):
    factory_contract = get_contract(FACTORY_CODE)
    factory_contract.initializeFactory(exchange_template.address, transact={})
    return factory_contract


@pytest.fixture
def omg_exchange(w3, exchange_abi, exchange_bytecode, exchange_factory, omg_token):
    exchange_factory.createExchange(omg_token.address, transact={})
    address = exchange_factory.getExchange(omg_token.address)
    return w3.eth.contract(address, abi=exchange_abi, bytecode=exchange_bytecode, ContractFactoryClass=VyperContract)


@pytest.fixture
def dai_exchange(w3, exchange_abi, exchange_bytecode, exchange_factory, dai_token):
    exchange_factory.createExchange(dai_token.address, transact={})
    address = exchange_factory.getExchange(dai_token.address)
    return w3.eth.contract(address, abi=exchange_abi, bytecode=exchange_bytecode, ContractFactoryClass=VyperContract)
