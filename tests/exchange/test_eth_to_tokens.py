...
def test_eth_to_tokens_payment(t, chain, utils, uni_token, uniswap_factory, uni_token_exchange, assert_tx_failed):
    ...
    # BUYER converts ETH to UNI
    uni_token_exchange.eth_to_tokens_payment(t.a3, 1, timeout, value=1*10**18, startgas=66000, sender=t.k2)
    # Updated balances of UNI exchange
    assert chain.head_state.get_balance(uni_token_exchange.address) == 6*10**18
    assert uni_token.balanceOf(uni_token_exchange.address) == 8336112037345781927
    # Updated balances of BUYER
...
def test_eth_to_tokens_exact_swap(t, chain, check_gas, utils, uni_token, uniswap_factory, uni_token_exchange, assert_tx_failed):
    uni_token.approve(uni_token_exchange.address, 10*10**18)
    uni_token_exchange.initialize(10*10**18, value=5*10**18)
    timeout = chain.head_state.timestamp + 300
    # Starting balances of UNI exchange
    ...
    # BUYER converts ETH to UNI
    uni_token_exchange.eth_to_tokens_exact_swap(1663887962654218073, timeout, value=2*10**18, startgas=74000, sender=t.k4)
    # Updated balances of UNI exchange
    assert chain.head_state.get_balance(uni_token_exchange.address) == 6*10**18
    assert uni_token.balanceOf(uni_token_exchange.address) == 8336112037345781927
    # Updated balances of BUYER
...
def test_eth_to_tokens_exact_payment(t, chain, utils, uni_token, uniswap_factory, uni_token_exchange, assert_tx_failed):
    uni_token.approve(uni_token_exchange.address, 10*10**18)
    uni_token_exchange.initialize(10*10**18, value=5*10**18)
    timeout = chain.head_state.timestamp + 300
    # Starting balances of UNI exchange
    ...
    # BUYER converts ETH to UNI
    uni_token_exchange.eth_to_tokens_exact_payment(t.a6, 1663887962654218073, timeout, value=2*10**18, startgas=75000, sender=t.k5)
    # Updated balances of UNI exchange
    assert chain.head_state.get_balance(uni_token_exchange.address) == 6*10**18
    assert uni_token.balanceOf(uni_token_exchange.address) == 8336112037345781927
    # Updated balances of BUYER
