def test_add_liquidity(t, chain, utils, uni_token, exchange_factory, uni_exchange, assert_tx_failed):
    ...
    # Can't addLiquidity without tokens
    ...
    # Liquidity provider (t.a0) addLiquiditys exchange
    ...
    # # Can't addLiquidity twice
    # assert_tx_failed(lambda: uni_exchange.addLiquidity(10*10**18, deadline, value=5*10**18))
...
def test_liquidity_pool(t, chain, utils, uni_token, exchange_factory, uni_exchange, assert_tx_failed):
    ...
    # First liquidity provider (t.a0) addLiquiditys exchange
    ...
    assert chain.head_state.get_balance(uni_exchange.address) == 6*10**18
    assert uni_token.balanceOf(uni_exchange.address) == 12*10**18
    # Can't addLiquidity while their are any shares left
    assert_tx_failed(lambda: uni_exchange.addLiquidity(2*10**18, deadline, value=1*10**18))
    # Mine block
    chain.mine()
    ...
    # Can addLiquidity again after all shares are divested
    uni_exchange.addLiquidity(2*10**18, deadline, value=1*10**18)
