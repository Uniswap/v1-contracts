def test_swap(t, chain, omg_token, omg_exchange, assert_tx_failed):
    ...
    assert chain.head_state.get_balance(t.a1) == 1*10**24 + 831248957812239452
...
def test_transfer(t, chain, omg_token, omg_exchange, assert_tx_failed):
    ...
    assert omg_token.balanceOf(t.a2) == 0
    assert chain.head_state.get_balance(t.a2) == 1*10**24 + 831248957812239452
...
def test_swap_exact(t, chain, omg_token, omg_exchange, assert_tx_failed):
    ...
    assert chain.head_state.get_balance(t.a1) == 1*10**24
    # BUYER converts ETH to UNI
    omg_exchange.tokenToEthSwapExact(831248957812239452, 3*10**18, deadline, sender=t.k1)
    # Updated balances of UNI exchange
    assert chain.head_state.get_balance(omg_exchange.address) == 4168751042187760548
...
def test_transfer_exact(t, chain, omg_token, omg_exchange, assert_tx_failed):
    ...
    assert chain.head_state.get_balance(t.a2) == 1*10**24
    # BUYER converts ETH to UNI
    omg_exchange.tokenToEthTransferExact(831248957812239452, 3*10**18, deadline, t.a2, sender=t.k1)
    # Updated balances of UNI exchange
    assert chain.head_state.get_balance(omg_exchange.address) == 4168751042187760548
    assert omg_token.balanceOf(omg_exchange.address) == 12*10**18 - 3
    # Updated balances of BUYER
