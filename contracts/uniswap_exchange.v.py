...
def initialize(token_amount: uint256) -> bool:
    ...
    log.Transfer(ZERO_ADDRESS, msg.sender, initial_shares)
    return True

# Lock up ETH and tokens at current price ratio
# Shares are minted proportional to liquidity added
...
def addLiquidity(min_shares: uint256, deadline: uint256) -> bool:
    ...
    shares_minted: uint256 = (eth_amount * share_total) / eth_pool
    assert shares_minted > min_shares
    token_amount: uint256 = (shares_minted * token_pool) / share_total
    ...
    log.Transfer(ZERO_ADDRESS, msg.sender, shares_minted)
    return True
...
def removeLiquidity(shares_burned: uint256, min_eth: uint256(wei), min_tokens: uint256, deadline: uint256) -> bool:
    ...
    share_total: uint256 = self.total_shares
    token_pool: uint256 = self.token.balanceOf(self)
    eth_amount: uint256(wei) = (shares_burned * self.balance) / share_total
    tokens_amount: uint256 = (shares_burned * token_pool) / share_total
    ...
    log.Transfer(msg.sender, ZERO_ADDRESS, shares_burned)
    return True
...
def __default__():
    ...
    token_pool: uint256 = self.token.balanceOf(self)
    fee: uint256(wei) = msg.value / 500
    new_token_pool: uint256 = ((eth_bal - msg.value) * token_pool) / (eth_bal - fee)
    token_amount: uint256 = token_pool - new_token_pool
...
def tokenToTokenSwapExact(token_addr: address, max_tokens_sold: uint256, tokens_bought: uint256, deadline: uint256, exact_output: bool) -> uint256:
    ...
    exchange_addr: address = Factory(self.factoryAddress).getExchange(token_addr)
    assert exchange_addr != ZERO_ADDRESS
    eth_pool_output: uint256(wei) = exchange_addr.balance
    token_pool_output: uint256 = Token(token_addr).balanceOf(exchange_addr)
